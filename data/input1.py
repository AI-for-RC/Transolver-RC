...
def process_elem_file(elem_file_path):
    ...
    # 处理307-612号节点，取1号和613号节点的平均值
    if 1836 >= 613:  # 确保节点数量足够
        for node_id in range(307, 613):
            for direction in range(3):
                avg_ratio = (node_rebar_ratios[0, direction] + node_rebar_ratios[612, direction]) / 2
                node_rebar_ratios[node_id - 1, direction] = avg_ratio
...
def process_beam_folder(folder_path):
    ...
    elem_file = os.path.join(folder_path, 'ELEM')

    # 处理NODE文件获取坐标
    coordinates = process_node_file(node_file)

    # 处理ELEM文件获取配筋率
    rebar_ratios = process_elem_file(elem_file)
...
def main():
    ...
    for item in os.listdir(root_directory):
        ...
        if os.path.isdir(item_path):
            ...
            elem_path = os.path.join(item_path, 'ELEM')
            if os.path.exists(node_path) and os.path.exists(elem_path):
    ...
    for i, folder_path in enumerate(tqdm(beam_folders, desc="处理梁文件夹")):
        # 获取当前梁的节点特征 [1836, 6]
        node_features = process_beam_folder(folder_path)

        # 将特征复制到所有49个荷载步
        data_matrix[i, :, :, :] = np.expand_dims(node_features, axis=2).repeat(49, axis=2)

    # 保存矩阵到文件
    output_file = "input1.npy"
