...
from utils.improved_loss import ImprovedLoss
...
#python improved_main.py --gpu 0 --model Transolver_Structured_Mesh_3D --n-hidden 128 --n-heads 8 --n-layers 8 --weight_decay 5e-5 --dropout 0.2 --lr 0.0003 --epochs 300 --max_grad_norm 0.1 --batch-size 64 --slice_num 64 --eval 0 --mlp_ratio 2 --resume 0 --save_name 13 --log_name train_log13
...
def main():
    ...
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        #optimizer,
        #T_max=args.epochs,  # 余弦周期，通常设为总轮数
        #eta_min=args.lr * 0.1  # 最小学习率，建议设为初始 lr 的 10%
    #)
    #scheduler = torch.optim.lr_scheduler.StepLR(
       # optimizer,
        #step_size=50,  # 每50个epoch衰减一次
        #gamma=0.5  # 衰减因子，每次衰减为原来的1/2
    #)
    myloss = ImprovedLoss(
        disp_weight=0.5,  # 可根据需求调整各部分权重
        stress_weight=1.5,
        strain_weight=1.5,
        abs_rel_ratio=0.5,  # 绝对误差与相对误差的平衡系数
        use_standardization=True  # 启用标准化（核心修改）
    )
    ...
    if eval:
        ...
        if args.resume:
            ...
            epoch_pbar = tqdm(range(checkpoint['epoch'], checkpoint['epoch'] + args.epochs), desc="Training Progress",
                              position=0)
            epochs = checkpoint['epoch'] + args.epochs
        else:
